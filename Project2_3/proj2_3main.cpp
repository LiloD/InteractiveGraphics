...
void Initialize(int argc, char* argv[])
{
	...
	glm::mat4 view;
	view = glm::lookAt(glm::vec3(-1.2f,0.0f,3.0f),glm::vec3(0.0f,0.0f,0.0f),glm::vec3(0.0f,1.0f,0.0f));
	glm::mat4 proj;
...
void InitWindow(int argc, char* argv[])
{
	...
	WindowHandle = glutCreateWindow("CS537 Interactive Graphics Project 2_3 by Zhizhuo Ding");
...
void IdleFunction(void)	
{
	...
	{
		glm::mat4 trans;
		trans = glm::rotate(glm::mat4(),-delta_x*(float)PI,glm::vec3(0.0f,1.0f,0.0f))*glm::rotate(glm::mat4(),-delta_y*(float)PI,glm::vec3(1.0f,0.0f,0.0f));

		shaderProgram->use();
		shaderProgram->SetUniform("model",trans);

		edgeShaderProgram->use();
		edgeShaderProgram->SetUniform("model",trans);

		trackShaderProgram->use();
		trackShaderProgram->SetUniform("model",trans);
	}
...
void MotionFunc(int x, int y)
{
	if(leftClick)
	{
		delta_x  = float(mouse_x-x)*0.5f;
		delta_y  = float(mouse_y-y)*0.5f;
	}
